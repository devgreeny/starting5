...
def _prompt_avatar(player_name, default):
    ...
    ``default`` is used if the input is empty or invalid.
    """
    try:
...
def generate_quiz_from_season(season, save_dir, manual_avatars=False, avatar_map=None):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate quizzes")
    parser.add_argument("--count", type=int, default=30, help="Number of quizzes to generate")
    parser.add_argument("--manual-avatars", action="store_true", help="Prompt for avatar numbers before saving")
    parser.add_argument("--avatar-mapping", help="JSON mapping of player_id to avatar number")
    ...
    if args.avatar_mapping:
        try:
            if Path(args.avatar_mapping).is_file():
                ...
                mapping = _json.loads(args.avatar_mapping)
        except Exception as e:
            print(f"Failed to parse avatar mapping: {e}")

    generate_quizzes_all_seasons(args.count, args.manual_avatars, mapping)
